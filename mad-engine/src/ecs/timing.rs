...
impl ECSTimingSystem {
    ...
    pub fn tick(&mut self) -> u64 {
        ...
        self.tick_count += 1;
        if now - self.last_rate_update >= 1000 {
        ...
        if self.current_tick_rate > 0 && self.current_tick_rate < self.goal_tick_rate {
            warn!(
                "Falling behind goal tick rate, current tick rate: {}, goal tick rate: {}",
                self.current_tick_rate, self.goal_tick_rate
            );
            return 0;
        }
        ...
        if self.delta_time < target_tick_time {
            info!(
                "Waiting for next tick, current tick rate: {}, goal tick rate: {}",
                self.current_tick_rate, self.goal_tick_rate
            );
            target_tick_time - self.delta_time
        } else {
            info!(
                "Already behind goal tick rate, current tick rate: {}, goal tick rate: {}",
                self.current_tick_rate, self.goal_tick_rate
            );
