...
impl Datastore{
    pub fn new(region: GridRegion) -> Self{
        Self{ region, layers: HashMap::new(), entities: HashMap::new() }
    }
    ...
    pub fn get_layer(&mut self, layer_type: String) -> Result<&LayerTable, anyhow::Error>{
        ...
        if !self.layers.contains_key(&layer_type){
            debug!("Layer type {} not found, creating new layer table", layer_type);
            let layer_table = LayerTable::new(layer_type.clone());
            self.layers.insert(layer_type.clone(), layer_table);
        }
        self.layers.get(&layer_type).ok_or(anyhow::anyhow!("Layer type {} not found", layer_type))
    }
    ...
    pub fn get_layer_mut(&mut self, layer_type: String) -> Result<&mut LayerTable, anyhow::Error>{
        ...
        self.layers.get_mut(&layer_type).ok_or(anyhow::anyhow!("Layer type {} not found", layer_type))
    }

    pub fn get_entity(&mut self, entity_type: String) -> Result<&EntityTable, anyhow::Error>{
        if !self.entities.contains_key(&entity_type){
            debug!("Entity type {} not found, creating new entity table", entity_type);
            let entity_table = EntityTable::new(entity_type.clone());
            self.entities.insert(entity_type.clone(), entity_table);
        }
        self.entities.get(&entity_type).ok_or(anyhow::anyhow!("Entity type {} not found", entity_type))
    }

    pub fn get_entity_mut(&mut self, entity_type: String) -> Result<&mut EntityTable, anyhow::Error>{
        if !self.entities.contains_key(&entity_type){
            debug!("Entity type {} not found, creating new entity table", entity_type);
            let entity_table = EntityTable::new(entity_type.clone());
            self.entities.insert(entity_type.clone(), entity_table);
        }
        self.entities.get_mut(&entity_type).ok_or(anyhow::anyhow!("Entity type {} not found", entity_type))
    }
}
