...
impl EntityTable{
    ...
    pub fn query(&self, query: EntityQuery) -> Vec<&EntityRow>{
        self.rows.iter().filter(|row| query.matches(&row.cell, row.entity_index)).collect()
    }
}
...
mod tests{
    ...
    fn test_query_list_in_cells(){
        ...
        entity_table.append_row(row);
        let rows = entity_table.query(EntityQuery::ListInCells(vec![GridCell::new(0, 0), GridCell::new(1, 1)]));
